...
COPY environment-${TOOL_VERSION}-nodefaults.yml environment-${TOOL_VERSION}-nodefaults.yml 
RUN mamba env update --file environment-${TOOL_VERSION}-nodefaults.yml --prefix /opt/conda/envs/${TOOL_NAME} \
 && mamba clean --all
